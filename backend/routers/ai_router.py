...
#C:\Users\talal\Projects\learn-ease-fyp\backend\routers\ai_router.py
...
async def http_generate_question_answers(
    ...
    # current_user: UserInDB = Depends(get_current_user) # Already in router dependencies
):
    """
    Receives text input and generates question and answer pairs.
    Questions are intended to be generated by a fine-tuned model (currently placeholder),
    and answers are generated by Gemini API based on those questions and the context.
    """
    try:
        ...
        # Re-raise HTTPExceptions that might come from the service layer
        # (e.g., model not loaded, Gemini API key missing, API call blocked)
        raise he
    except Exception as e:
