...
const CreateGroupModal = ({ isOpen, onClose, onGroupCreated }: CreateGroupModalProps) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      onGroupCreated(newGroup); // Pass the new group back to the inbox list
      resetForm();
      onClose();
    } catch (err: any) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Create a New Study Group">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input 
            ...
            placeholder="e.g., Computer Networks Final Cram"
        ...
        {/* Description */}
        <div>
          <label htmlFor="groupDescription" className="block text-sm font-medium text-slate-700 dark:text-slate-300">
        ...
        <div className="flex justify-end pt-4">
          ...
          >
            {isSubmitting ? <SpinnerIcon /> : "Create Group"}
          </button>
        </div>
      </form>
    </Modal>
